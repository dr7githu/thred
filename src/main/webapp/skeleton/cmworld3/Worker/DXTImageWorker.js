...
onmessage = function (event)
{
   ...
   if (firstLoad == true)
   {
       var cmworld3BaseRoot = event.cmworld3BaseRoot;

       importScripts(cmworld3BaseRoot + 'cm_compile.js');
       importScripts(cmworld3BaseRoot + 'lib/image/bmpdecode.js');
       importScripts(cmworld3BaseRoot + 'lib/image/jpegdecode.js');
       importScripts(cmworld3BaseRoot + 'lib/image/zlib.js');
       importScripts(cmworld3BaseRoot + 'lib/image/png.js');
   ...
   try
   {
      if (edata.command == 'start')
      {
         ...
         {
            downloaddata = new Uint8Array(xhr.response || xhr.mozResponseArrayBuffer);
            var imagedata = null;

            if (downloaddata)
            {
               // 이미지 형식을 판단하자.
               if (downloaddata[0] == 0x89)
               {
                  // PNG
                  var png = new PNG();
                  png.load(downloaddata);
                  imagedata = png.decode();

               }
               else if (downloaddata[0] == 0xFF)
               {
                  // JPEG
                  imagedata = jpegdecode(downloaddata);
               }
               else if (downloaddata[0] == 0x42)
               {
                  // BMP
                  imagedata = bmpdecode(downloaddata);
               }

               if (imagedata)
               {
                  var compressed = compress(imagedata.width, imagedata.height, imagedata.data);
